...
- **`lib/bindeps.js`** - Resolves the yt-dlp / ffmpeg binaries: PATH first, then PLDL's managed dir (`~/.pldl/bin`), then downloads on first run. yt-dlp uses a self-contained per-platform release (reliable); ffmpeg uses best-effort static-build sources (evermeet on macOS, yt-dlp's FFmpeg-Builds on win/linux) and degrades gracefully if unavailable. `server.js` calls `ensureBinaries()` at startup (after `listen`, in the background), publishes the resolved yt-dlp path via `process.env.PLDL_YTDLP` (read by `lib/playlist.js` and `lib/archive.js`) and the ffmpeg dir via `process.env.PLDL_FFMPEG_DIR` (passed to yt-dlp as `--ffmpeg-location`). The three yt-dlp-dependent endpoints (`playlist-info`, `download`, `download-recovered`) return **503** until `deps.status === 'ready'`; setup progress streams as SSE `deps` events and `/api/health` reports `deps: { status, ffmpeg, error }`.
