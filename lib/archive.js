...
function runYtdlp(args) {
  // Use PLDL's resolved binaries (set by the server's dependency manager),
  // falling back to bare names when run standalone.
  const bin = process.env.PLDL_YTDLP || 'yt-dlp';
  const ffmpegArgs = process.env.PLDL_FFMPEG_DIR ? ['--ffmpeg-location', process.env.PLDL_FFMPEG_DIR] : [];
  return new Promise((resolve) => {
    const p = spawn(bin, [...ffmpegArgs, ...args]);
    let stderr = '';
